...
from .globals import RANDOM_SEED, DEVICE, ALWAYS_RB, SAMPLE_RB, ACTIONS, STOCHASTIC_POLICY, MULTIPLE_AGENTS, COIN_LEADING
from .features import find_ideal_path
...
def act(self, game_state: dict) -> str:
    ...
    if self.train and COIN_LEADING:
        self.logger.debug(f"Forced to choose route to nearest coin.")
        return find_ideal_path(game_state['self'][3], game_state['coins'], game_state['field'])
    ...
    if self.train and random.random() < self.eps:
        ...
        with torch.no_grad():
            ...
            #Q_values = self.model.state_to_features(game_state, self).flatten().squeeze()
            Q_values = self.model(self.MODEL_TYPE.state_to_features(game_state,self).flatten().squeeze())
