...
from .features import find_ideal_path, get_blast_coords,\
                    get_safety_feature, get_coin_feature, get_enemy_agent_feature,\
                    enemy_in_blast_coords, find_path_to_target, get_first_step_from_path
...
#afterwards update entries !=0 from temp table in original table
    ...
    def train_step(self, agent, transitions):
        ...
        Q_old = Q_old[a_idx] #  1 #Q(s_t, a_t)

        target = reward - Q_old

        if transition_new is not None:
            _, new_action, _, _, _ = transition_new
            Q_new = agent.model(new_state)[ACTIONS_DICT[new_action]] # Q(s_{t+1}, a_{t+1})
            target += GAMMA * Q_new 
...
#afterwards update entries !=0 from temp table in original table
    ...
    def train_step(self, agent, transitions):
        ...
        # TODO: how to send tensors to GPU if available
        transition_old, transition_new = transitions[0], transitions[-1]
        old_state, action, new_state, reward, new_state_dict = transition_old
        ...
        if transition_new is not None:
            _, new_action, _, _, _ = transition_new
            Q_new = agent.model(new_state)[ACTIONS_DICT[new_action]] # Q(s_{t+1}, a_{t+1})
            target += GAMMA**len(transitions) * Q_new 

            rewards = [transition.reward for transition in transitions]
            G = np.sum([GAMMA**j *r for j, r in enumerate(rewards)])

        if transition_new is None:
            rewards = [transition.reward for transition in transitions[:-1]]
            G = np.sum([GAMMA**j *r for j, r in enumerate(rewards)])

        target += G
        # target = G + GAMMA**N * Q_new - Q_old
        self.table[tuple(old_state)][a_idx] = Q_old + LR * target

        if transition_new is None and len(transitions)>2:
            self.train_step(agent, transitions[1:])